...
# Use bundled jq if available (set by parent script), otherwise fall back to system jq
if [ -z "$JQ" ]; then
    # Try to find bundled jq relative to this script
    SCRIPT_DIR_LIB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [ -f "$SCRIPT_DIR_LIB/../bin/jq-binary" ] && [ -x "$SCRIPT_DIR_LIB/../bin/jq-binary" ]; then
        JQ="$SCRIPT_DIR_LIB/../bin/jq-binary"
    elif command -v jq &> /dev/null; then
        JQ="jq"
    else
        JQ="jq"  # Will fail with clear error if jq not found
    fi
fi
